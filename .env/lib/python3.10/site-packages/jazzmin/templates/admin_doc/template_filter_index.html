            ...
            <div class="nav flex-column nav-tabs h-100" id="filter-tab" role="tablist" aria-orientation="vertical">
                {% for library in filter_libraries %}
                    <a class="nav-link" id="tabs-{% firstof library.grouper built_in_filters %}-tab" data-toggle="pill"
                       ...
                       aria-controls="tabs-{% firstof library.grouper built_in_filters %}" aria-selected="true">
                        <h6>{% firstof library.grouper _("Built-in filters") %}</h6></a>
                {% endfor %}
            </div>
        </div>
                    ...
                    <div class="tab-pane text-left fade{% if forloop.first %} show active{% endif %}"
                        ...
                        {% for view in ns_views.list|dictsort:"url" %}
                            {% ifchanged %}
                                ...
                                <p class="small quiet">
                                    {% blocktrans with view.full_name as full_name and view.url_name as url_name %}View
                        ...
                        {% if library.grouper %}<p class="small quiet">
                            {% blocktrans with code="{"|add:"% load "|add:library.grouper|add:" %"|add:"}" %}To use
                                these filters, put <code>{{ code }}</code> in your template before using the
